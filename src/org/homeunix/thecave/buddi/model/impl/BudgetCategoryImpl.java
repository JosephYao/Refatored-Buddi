...
public class BudgetCategoryImpl extends SourceImpl implements BudgetCategory {
	 ...
	 * @return
	 */
	public long getAmountFromBudgetPeriodContainingDate(Date periodDate) {
	...
	private double getAmountForOverlappingDays(Period period, BudgetPeriod budgetPeriod) {
		long amount = getAmountFromBudgetPeriod(budgetPeriod);
		long daysInPeriod = budgetPeriod.getDayCount();
		long daysBetween = period.getOverlappingDayCount(budgetPeriod.getPeriod()); 
	...
	public void setAmount(Date periodDate, long amount) {
		if (getAmountFromBudgetPeriodContainingDate(periodDate) != amount)
			setChanged();
