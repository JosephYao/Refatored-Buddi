...
public class FilteredLists {
	 ...
	 *
	 */
	public static class FilteredTransactionList extends BuddiFilteredList<Transaction> {
		private final Source source;
		private String searchText;
		private TransactionDateFilterKeys dateFilter; 

		public FilteredTransactionList(DataModel model, Source source){
			super(model, model.getTransactions());
			this.source = source;
		}
...
//			}
		...
		private boolean acceptText(Transaction t) {
			...
			return (searchText == null
					...
					|| APICommonFormatter.getFormattedCurrency(t.getAmount()).replaceAll("[^\\d" + decimal + "]", "").contains(searchText.toLowerCase()))
					|| APICommonFormatter.getDateFormat().format(t.getDate()).toLowerCase().contains(searchText.toLowerCase());
	 ...
	 * @author wyatt
	 */
	public static class FilteredAccountList extends BuddiFilteredList<Account> {
		private final Type type;

		public FilteredAccountList(DataModel model, Type type) {
	 ...
	 * @author wyatt
	 */
	public static class FilteredTypeList extends BuddiFilteredList<Type> {
		private final DataModel model;

		public FilteredTypeList(DataModel model) {
	...
	public static class FilteredBudgetCategoryChildrenList extends BuddiFilteredList<BudgetCategory> {
		private final BudgetCategory parent;
		
		public FilteredBudgetCategoryChildrenList(DataModel model, BudgetCategory parent) {
	 ...
	 * @author wyatt
	 */
	public static class FilteredBudgetCategoryList extends BuddiFilteredList<BudgetCategory> {
		private final BudgetCategory parent;

		public FilteredBudgetCategoryList(DataModel model, BudgetCategory parent) {
			super(model, model.getBudgetCategories());
			this.parent = parent;
		}
