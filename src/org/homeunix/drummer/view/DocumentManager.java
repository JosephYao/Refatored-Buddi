...
import net.sourceforge.buddi.api.manager.BuddiUtil;
	...
	public ReturnCodes newDataFile(File file){
		while (file == null){
			...
			if (jfc.showSaveDialog(null) == JFileChooser.APPROVE_OPTION){
				...
				// file was selected.
				else if (!BuddiUtil.isFolderWritable(jfc.getSelectedFile().getParentFile())){
	...
	public File showExportFileDialog(File file, String title, FileFilter filter, String extension){
		while (file == null){
			...
			if (jfc.showSaveDialog(null) == JFileChooser.APPROVE_OPTION){
				...
				//Check that we have write permission to the folder where this
				// file was selected.
				if (!BuddiUtil.isFolderWritable(jfc.getSelectedFile().getParentFile())){
					String[] options = new String[1];
					options[0] = Translate.getInstance().get(TranslateKeys.BUTTON_OK);
